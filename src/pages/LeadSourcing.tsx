...
type SourcingStatus = { coverage: SourcingCoverage[]; active_job: { language: Language; search_query: string; state: string } | null; next_markets: Partial<Record<Language, { language: Language; category: string; city: string } | null>> };
...
export default function LeadSourcing() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">{(["sv", "en"] as Language[]).map((lang) => { const item = sourcingStatus?.coverage.find((coverage) => coverage.language === lang); const next = sourcingStatus?.next_markets?.[lang]; return <Card key={lang} className="p-5"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{lang === "sv" ? "Svensk sourcing-status" : "English sourcing status"}</p><p className="mt-1 text-sm text-muted-foreground">{item ? `${item.stock}/${item.target} leads · audit ${item.audit_backlog}/${item.backlog_cap} · granskning ${item.review_backlog}/${item.backlog_cap}` : "Läser status…"}</p></div><Badge variant={item?.should_source ? "default" : "secondary"}>{item?.should_source ? "Behöver leads" : "Väntar"}</Badge></div><p className="mt-3 text-sm text-muted-foreground">{item?.reason ?? "Ingen status ännu."}</p>{sourcingStatus?.active_job && <p className="mt-2 text-xs text-muted-foreground">Aktivt jobb: {sourcingStatus.active_job.search_query}</p>}{!sourcingStatus?.active_job && next && item?.should_source && <p className="mt-2 text-xs text-muted-foreground">Nästa: {next.category} — {next.city}</p>}</Card>})}</div>
